...
function debugTables() {
    ...
    const materialsTable = document.getElementById('materialsUsedTable');
    const summaryTable = document.getElementById('summaryTable');
    
    console.log('Materials table found:', !!materialsTable);
    console.log('Summary table found:', !!summaryTable);
    
    if (materialsTable) {
        const rect = materialsTable.getBoundingClientRect();
        console.log('Materials table size:', rect.width, 'x', rect.height);
        console.log('Materials table HTML:', materialsTable.outerHTML.substring(0, 200));
    }
    
    if (summaryTable) {
        const rect = summaryTable.getBoundingClientRect();
        console.log('Summary table size:', rect.width, 'x', rect.height);
        console.log('Summary table HTML:', summaryTable.outerHTML.substring(0, 200));
    }
    
    const sideContainer = document.querySelector('.side-by-side-tables');
    if (sideContainer) {
        const rect = sideContainer.getBoundingClientRect();
        console.log('Side container size:', rect.width, 'x', rect.height);
    }
