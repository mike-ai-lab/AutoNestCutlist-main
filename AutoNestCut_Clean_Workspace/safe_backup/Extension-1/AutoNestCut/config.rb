module AutoNestCut
  module Config
    ...
    def self.get_grain_direction_from_definition(component_definition)
      ...
      if height > width * 1.5
        return 'L'  # Length (vertical)
      elsif width > height * 1.5
        return 'W'  # Width (horizontal)
      end
    ...
    def self.get_edge_banding_from_definition(component_definition)
      # Analyze face materials to determine edge banding
      analyze_face_materials_for_edge_banding(component_definition)
    end
    
    def self.analyze_face_materials_for_edge_banding(component_definition)
      return 'None' unless component_definition.entities
      ...
      return 'None' if faces.length < 2
      
      # Sort faces by area to identify main faces vs edge faces
      faces.sort_by! { |f| -f.area }
      
      # Get main panel material from largest faces
      main_material = nil
      faces[0..1].each do |face|
      ...
      return 'None' unless main_material
      
      # Count edge faces (smaller faces) with matching material
      banded_edges = 0
      edge_faces = faces[2..-1] || []
      
      edge_faces.each do |face|
        if face.material == main_material || face.back_material == main_material
          banded_edges += 1
        end
      end
