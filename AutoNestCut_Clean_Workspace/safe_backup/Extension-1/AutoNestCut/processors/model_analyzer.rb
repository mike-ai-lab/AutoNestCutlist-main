...
module AutoNestCut
  class ModelAnalyzer
    ...
    def extract_parts_from_selection(selection, progress_callback = nil)
      definition_counts = {}
      ...
      @selected_entities.each_with_index do |entity, index|
        if @progress_callback
          progress = (index.to_f / total_entities * 30).round(1)
          @progress_callback.call("Building component hierarchy... #{index + 1}/#{total_entities}", progress)
        end
        ...
        # Micro-sleep every 10 entities to prevent freezing
        sleep(0.001) if index % 10 == 0
      end
