...
function renderReport() {
    ...
    if (uniqueBoardTypesTable) {
        ...
        if (g_reportData.unique_board_types) {
            g_reportData.unique_board_types.forEach(board_type => {
                ...
                // Extract dimensions from string like "2440.0 x 1220.0 mm"
        ...
        uniqueBoardTypesTable.innerHTML = html;
        addCopyButton('Unique Board Types', 'uniqueBoardTypesTable');
    }

    const sheetInventoryTable = document.getElementById('sheetInventoryTable');
    if (sheetInventoryTable) {
        let html = `<caption>Sheet Inventory Summary</caption><tr><th>Material</th><th>Dimensions</th><th>Count</th><th>Total Area</th><th>Price/Sheet</th><th>Total Cost</th></tr>`;
        if (g_reportData.unique_board_types) {
            g_reportData.unique_board_types.forEach(board_type => {
                const boardCurrency = board_type.currency || currency;
                const boardSymbol = currencySymbols[boardCurrency] || boardCurrency;
        ...
        console.error('uniqueBoardTypesTable element not found');
    }
    ...
    if (costBreakdownContainer && g_reportData.parts_placed) {
        ...
        let costHtml = `<div class="cost-breakdown-independent">`;
        ...
        Object.entries(materialCosts).forEach(([material, data]) => {
            ...
            costHtml += `<div class="cost-line-independent">${material}: ${data.count} parts - ${symbol}${data.cost.toFixed(2)} (${percentage.toFixed(1)}%)</div>`;
        });
        
        let costTableHtml = `<table><tr><th>Material</th><th>Parts</th><th>Cost</th><th>Percentage</th></tr>`;
        Object.entries(materialCosts).forEach(([material, data]) => {
            const percentage = totalPartsCost > 0 ? (data.cost / totalPartsCost * 100) : 0;
            const symbol = currencySymbols[data.currency] || data.currency;
    ...
    let boardsHtml = `<tr><th>Board#</th><th>Material</th><th>Size</th><th>Parts</th><th>Efficiency</th></tr>`;
    if (g_reportData.boards) {
        g_reportData.boards.forEach(board => {
            // Extract dimensions from string like "2440.0 x 1220.0 mm"
            const dimMatch = (board.stock_size_mm || board.stock_size || '2440 x 1220 mm').match(/(\d+(?:\.\d+)?)\s*x\s*(\d+(?:\.\d+)?)/i);
            const width = dimMatch ? parseFloat(dimMatch[1]) / unitFactors[reportUnits] : 2440 / unitFactors[reportUnits];
            const height = dimMatch ? parseFloat(dimMatch[2]) / unitFactors[reportUnits] : 1220 / unitFactors[reportUnits];
            const sizeStr = `${formatNumber(width, reportPrecision)} × ${formatNumber(height, reportPrecision)} ${reportUnits}`;
            
            boardsHtml += `
                <tr>
                    <td>${board.board_number}</td>
                    <td title="${board.material}">${board.material}</td>
                    <td>${sizeStr}</td>
                    <td class="total-highlight">${board.parts_count}</td>
                    <td>${formatNumber(board.efficiency, reportPrecision)}%</td>
                </tr>
            `;
        });
    }
    if (boardsTable) {
        boardsTable.innerHTML = boardsHtml;
        addCopyButton('Boards Summary', 'boardsTable');
    } else {
        console.error('boardsTable element not found');
    }
    ...
    if (partsTable) {
        ...
        const costAnalysisContainer = document.createElement('div');
        costAnalysisContainer.className = 'cost-analysis-isolated';
        costAnalysisContainer.innerHTML = costAnalysisHtml;
        
        partsTable.parentNode.insertBefore(costAnalysisContainer, partsTable);
