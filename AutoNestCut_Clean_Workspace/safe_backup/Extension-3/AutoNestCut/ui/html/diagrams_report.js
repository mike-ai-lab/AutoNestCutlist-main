...
function receiveData(data) {
    ...
    // Update header values based on received data settings
    if (data.settings) {
        window.currentUnits = data.settings.units || window.currentUnits;
        window.currentPrecision = data.settings.precision ?? window.currentPrecision;
        window.defaultCurrency = data.settings.currency || window.defaultCurrency;
        window.currentAreaUnits = data.settings.areaUnit || window.currentAreaUnits; // Assume areaUnit is also in settings

        // Update display elements directly if they exist
        const headerUnits = document.getElementById('headerUnits');
        if (headerUnits) headerUnits.textContent = window.currentUnits;
        const headerPrecision = document.getElementById('headerPrecision');
        if (headerPrecision) headerPrecision.textContent = window.currentPrecision;
        const headerCurrency = document.getElementById('headerCurrency');
        if (headerCurrency) headerCurrency.textContent = window.defaultCurrency;
        const headerAreaUnit = document.getElementById('headerAreaUnit');
        if (headerAreaUnit) headerAreaUnit.textContent = getAreaUnitLabel();
    }
...
// Function to handle initial data from main.html
// This function was missing and caused the "receiveInitialData is not defined" error
function receiveInitialData(data) {
    console.log("Received initial data:", data);
    // This function will be called by the Ruby backend to send the initial report data.
    // It should trigger the rendering of the report and diagrams.
    receiveData(data); // Delegate to the existing receiveData function
}
...
function renderDiagrams() {
    ...
    g_boardsData.forEach((board, boardIndex) => {
        ...
        const units = window.currentUnits || 'mm';
        const precision = window.currentPrecision ?? 1; 
        
        const width = board.stock_width / window.unitFactors[units];
        const height = board.stock_height / window.unitFactors[units];
        
        // Removed `units` from dimension values in info string
        info.innerHTML = `Size: ${formatNumber(width, precision)}×${formatNumber(height, precision)} ${units}<br>
        ...
        canvas.drawCanvas = function() { // Bind drawing function to canvas
            ...
            parts.forEach((part, partIndex) => {
                ...
                if (partWidth > 50) {
                    ...
                    ctx.fillText(`${formatNumber(partDisplayW, 0)}`, partX + partWidth / 2, partY + 5); // Removed unit
                }
                ...
                if (partHeight > 50) {
                    ...
                    ctx.fillText(`${formatNumber(partDisplayH, 0)}`, 0, 0); // Removed unit
                    ctx.restore();
                }
...
function renderReport() {
    ...
    if (materialsUsedTable && g_reportData.unique_board_types) {
        ...
        g_reportData.unique_board_types.forEach(board_type => {
            ...
            materialsUsedTable.innerHTML += `<tr><td>${board_type.material}</td><td>${boardSymbol}${(board_type.price_per_sheet || 0).toFixed(2)}</td></tr>`;
        });
    }
...
function handleCanvasHover(e, canvas) {
    ...
    if (canvas.partData) {
        for (let partData of canvas.partData) {
            if (x >= partData.x && x <= partData.x + partData.width &&
                y >= partData.y && y >= partData.y + partData.height) { // Fixed y condition
                hovering = true;
                break;
            }
        }
...
function showPartModal(part) {
    ...
    const projToggle = document.getElementById('projectionToggle'); // Ensure this element exists
    ...
    const existingErrorDiv = modalCanvas.parentNode.querySelector('.threejs-error-message');
    if (existingErrorDiv) {
        existingErrorDiv.remove();
    }
    modalCanvas.style.display = 'block'; // Show canvas by default
    ...
    try {
        ...
        errorDiv.className = 'threejs-error-message'; // Add class for easy removal
        errorDiv.innerHTML = '<p style="color: #666; text-align: center; padding: 20px;">3D visualization not available (WebGL not supported or error)</p>';
    ...
    if (projToggle) { // Only show/hide if the element exists
        if (!hasError) {
            projToggle.style.display = 'inline-block'; // Show button
            ...
            projToggle.onclick = () => {
                ...
                // Calculate size based on the object's dimensions to fit it well
                const boundingBox = new THREE.Box3().setFromObject(cube);
                const size = Math.max(boundingBox.max.x - boundingBox.min.x, boundingBox.max.y - boundingBox.min.y, boundingBox.max.z - boundingBox.min.z) * 1.5; // Add some padding
                ...
                camera.position.set(size, size, size); // Reset camera position
                camera.lookAt(0,0,0);
                controls.object = camera;
                controls.update();
            };
        } else {
            projToggle.style.display = 'none'; // Hide button if 3D failed
        }
    }
...
function initThreeJS(part, canvas) {
    // Clear previous scene elements if re-initializing
    if (scene) {
    ...
    // Calculate aspect ratio dynamically based on canvas current size
    const aspect = canvas.clientWidth / canvas.clientHeight;
    camera = new THREE.PerspectiveCamera(75, aspect, 0.1, 1000);
    camera.position.set(5, 5, 5);
    
    // Check for existing renderer and dispose if found to prevent WebGL context issues
    if (renderer) {
        renderer.dispose();
    ...
    renderer.setSize(canvas.clientWidth, canvas.clientHeight); // Use clientWidth/Height for accurate size
    ...
    // Dynamic resizing for modal canvas
    const resizeObserver = new ResizeObserver(() => {
        if (renderer && camera) {
            ...
            if (camera.isPerspectiveCamera) {
                ...
                const size = Math.max(cube.geometry.boundingBox.max.x - cube.geometry.boundingBox.min.x, cube.geometry.boundingBox.max.y - cube.geometry.boundingBox.min.y, cube.geometry.boundingBox.max.z - cube.geometry.boundingBox.min.z) * 1.5;
                camera.left = -size * newAspect / 2;
                camera.right = size * newAspect / 2;
            }
            camera.updateProjectionMatrix();
            renderer.setSize(canvas.clientWidth, canvas.clientHeight);
            animate(); // Request a redraw
        }
    });
...
function setupOrbitControls() {
    // Dispose previous controls to avoid memory leaks and multiple active controls
    if (controls) {
        controls.dispose();
    }
...
function animate() {
    ...
    // Only request animation frame if the modal is still open
...
function handleContextRestored() {
    ...
    // Re-initialize Three.js scene, camera, renderer, etc.
    if (currentPart && document.getElementById('partModal').style.display === 'block') {
        showPartModal(currentPart); // Re-render the modal with the current part
    }
}
...
// Global function for `openTreeStructureModal` as it's called from main.html
function openTreeStructureModal() {
    const modal = document.getElementById('treeModal');
    if (modal) {
        modal.style.display = 'block';
        renderTreeStructure(); // Ensure tree is rendered when opened
    }
}

function closeTreeStructureModal() {
    const modal = document.getElementById('treeModal');
    if (modal) {
...
// Helper to get the correct area unit label (e.g., m², in²)
function getAreaUnitLabel() {
    const units = window.currentAreaUnits || 'm2';
    const areaLabels = { mm2: 'mm²', cm2: 'cm²', m2: 'm²', in2: 'in²', ft2: 'ft²' };
    return areaLabels[units] || 'm²';
}
